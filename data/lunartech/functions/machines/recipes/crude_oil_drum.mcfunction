...
execute if score bottle_of_crude_oil count matches 3.. run tag @s add one

execute if score bottle_of_crude_oil count matches 3.. run function lunartech:items/crude_oil_drum
execute if score bottle_of_crude_oil count matches 3.. run tag @e[tag=!restrict,nbt={Item:{tag:{bottle_of_crude_oil:1}}},distance=..1,limit=3] add delete
